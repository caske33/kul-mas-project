...
We decided to run two experiments. In the first one, we varied the number of drones, warehouses, initial clients and \textit{extra clients}, i.e. the clients that appear randomly after the start of the simulation. The different values can be found in table~\ref{tbl:independent}. \\
As figure~\ref{fig:dynamicclients-profit} and \ref{fig:dynamicclients-profit-largeNbDrones} show, DynCNET has troubles with breakdown when there are many clients compared to the number of drones. Therefore, the second experiment is aiming at exploring these circumstances for a large number of drones.

Multiple variables were measured. They can be found in table~\ref{tbl:dependent}.
All raw data is kept and can be found in attachment to this report. The values for other variables can be found in table~\ref{tbl:other}.

All experiments were run using RinSim 4.1.0 on computers at the P\&O lab at the Department of Computer Science. These computers have a 3.4 GHz quad-core processor and 8 GB of RAM. 150 simulations were performed for each combination of values for the independent variables\footnote{For a total of 1417500 and 351000 simulations respectively for experiment 1 and 2.}
...
\begin{table}[hp]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Independent variables and their ranges (with inclusive boundaries)\commentY{wat vindt ge van notatie "all 3" voor protocol? Moet die er zelfs bij?}}
...
\begin{table}[htp]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Dependant variables and their meaning\commentY{wat doen we met dependant variables die in csv voorkomen maar niet in de grafieken? Is het ok dat ik sommige variabelen hernoemd heb om in tabel te passen?}}
	\label{tbl:dependent}
\end{table}
\begin{table}[htp]
	\begin{center}
