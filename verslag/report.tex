...
\commentV{uitleggen dat we deze niet geimplementeerd hebben}
...
Across the world several warehouses are located. For simplification, these warehouses have an infinite stock of every package that a client could potentially order. Drones can pick up packages at these warehouses for a price that varies per warehouse in a range of 80 to 120\% of that package's market price. These warehouses are divided over the whole area in such a way that a clustering of warehouses is avoided as much as possible. To achieve niet, the number of warehouse is split in powers of two (e.g. 5 = 4 ($2^2$) + 1 ($2^0$)). For every power of two, the area is divided in the that number of equiareal rectangles, each containing one warehouse at a uniformly random position.
...
\commentY{iets zeggen over "uniformly" random? \proposalV{clients that appear randomly afterwards, uniformly distributed over time.}}
...
\begin{table}[hp]
	\begin{center}
		\begin{tabular}{ lcc }
			...
			protocol & all 3 & all 3 \\
			\hline
		\end{tabular}
	\end{center}
	\caption{Independent variables and their ranges (with inclusive boundaries)\commentY{wat vindt ge van notatie "all 3" voor protocol? Moet die er zelfs bij? \commentV{misschien eerder in de tekst vermelden?}}}
...
\begin{table}[htp]
	\begin{center}
		\begin{tabular}{ p{0.4\columnwidth}p{0.5\columnwidth} }
			...
			profit & The profit the company made (in \euro{}) \\
			...
			averageDistance & The average distance (in km) a drone travelled\\
			maximumOrders & The maximum orders 1 drone delivered\\
			estimatedProfit & The sum of the estimated profits of the accepted contracts (in \euro{})\\
			estimatedCrashes & The estimated number of crashes (for the accepted contracts) \\
			nbMessages & The number of messages exchanged \\
			nbCallsForProposal & The number of task announcements (on average) \\
			switchesPerDrone & The (average) number of times a drone switched contracts \\
			switchesPerClient & The (average) number of times a client switched contracts \\
			\hline
		\end{tabular}
	\end{center}
	\caption{Dependant variables and their meaning\commentY{wat doen we met dependant variables die in csv voorkomen maar niet in de grafieken? \commentV{niet vermelden zou ik zeggen }Is het ok dat ik sommige variabelen hernoemd heb om in tabel te passen? \commentV{als ook header in csv hernoemd (+eventueel code): ok}}}
	\label{tbl:dependent}
\end{table}
\begin{table}[htp]
	\begin{center}
...
\subsection{Results}
\taskS{Update maxlimiet van figures hieronder met laatste figuur!}
...
\commentY{Er is nog plaats voor 1 figuur.\proposalY{Figuur~\ref{fig:dynamicclients-switchesdrone-grid2}}}
\MASplotThree
...
\MASplotThree{dynamicclients-profit}{The influence of the number of clients that appear after the start of the simulation on the profit}
...
{drones-deliverytime}{The influence of the number of drones on the delivery time\commentY{omzetten naar seconden ipv milliseconden?}}

\MASplotThree{dynamicclients-delivered-grid2}{The influence of the number of clients that appear after the start of the simulation on the number of clients that are not delivered (in the second experiment) }
...
\subsection{Analysis}
\commentY{Echte analyse doen of ook echt expliciet vergelijken met onze hypothese? \commentV{we moeten ook wel zeggen of onze hypothese nu klopte of niet}}
\commentY{In volgorde van de research questions laten of herordenen? \commentV{volgorde behouden is duidelijkste denk ik, er is niet echt een 'verhaal' waarbij het ene stuk in het andere overgaat}}
...
Figure~\ref{fig:drones-profit} makes it clear that the number of drones has an influence on which protocol maximizes profit. When there are many drones (7 or more), DynCNET is better than CNCP, which outperforms CNET. For an average number of drones (4 to 6), CNCP outperforms CNET and DynCNET. For 3 drones or less, there is no difference between CNET and CNCP, but both outperform DynCNET. As figure~\ref{fig:dynamicclients-profit} and \ref{fig:dynamicclients-profit-largeNbDrones} show, no breakdown happens when enough drones are present. This explains the different relative performance of DynCNET depending on the number of drones \commentV{hoe? bij te weinig drones -> breakdown en dus lagere profit}.

Figure~\ref{fig:drones-deliverytime} shows the influence of the number of drones on the delivery time. For 1 drone, DynCNET delivers the fastest, while there is no significant difference between CNET and CNCP\footnote{in case of 1 drone, both protocols don't differ much \commentV{is dat niet hetzelfde als wat in de tekst staat?}}. If two or more drones are present in the world, CNCP will deliver faster than CNET, which will deliver faster than DynCNET.

The impact of the protocol on the number of clients that don't get their package delivered also depends on how many clients and drones there are. \commentV{figuren?} For a small number of clients (less than 20 clients initially and less than 50 clients after the start) or a large number of drones (10 drones), in CNCP the most packages are delivered, followed by CNET and DynCNET. When there are 9 drones, no significant difference is found between CNET and DynCNET. \commentV{CNCP?} When there are 8 drones, DynCNET outperforms CNET. For a smaller number of drones (4 drones), CNET outperforms DynCNET again. \commentV{wat met 5-7?} Finally, for a very small number of drones (1 or 2 drones), no significant difference was found between CNET and CNCP. \commentV{DynCNET?} In our first experiment, CNCP always outperformed DynCNET. \commentV{in het algemeen of voor 1/2 drones?} However, figure~\ref{fig:dynamicclients-delivered-grid2} shows that when the number of clients increases, DynCNET starts to outperform CNCP. No range was found, in neither experiment, where CNET outperforms CNCP. Only when there are 1 or 2 drones in the world does CNET keep up with CNCP. This is because both protocols act very similarly when there are few drones.

When there are a small number of clients (5 initial clients and 10 extra clients), no significant difference in the number of exchanged messages could be found between CNET and CNCP. However, in general \commentV{dus voor avg/grote aantallen klanten?}, CNCP needs more messages than CNET. DynCNET always needs more messages than the other two protocols.

As figures~\ref{fig:drones-profit}--\ref{fig:dynamicclients-profit} show, a company can make a profit using any of the three protocols, given the right number of drones and warehouses for the given number of clients in the world.
